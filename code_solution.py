...
# Densité finale après propagation
densite_finale = re**2 + im**2
# Zones de calcul
zone_reflexion = o < 0.8   # à gauche du puits (réflexion)
zone_transmission = o > 0.9  # à droite du puits (transmission)

# Calcul de l'intégrale (approximation trapézoïdale)
R = np.trapz(densite_finale[zone_reflexion], o[zone_reflexion])
T = np.trapz(densite_finale[zone_transmission], o[zone_transmission])
total = np.trapz(densite_finale, o)
# Affichage des résultats
print(f"Réflexion R = {R:.4f}")
print(f"Transmission T = {T:.4f}")
print(f"Somme R + T = {R+T:.4f} (Total = {total:.4f})")
# it=0
# for i in range(1, nt):
#     if i % 2 != 0:
#         b[1:-1]=im[1:-1]
